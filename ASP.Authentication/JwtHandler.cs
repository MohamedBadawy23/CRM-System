...
public class JwtHandler
{
    ...
    private List<Claim> GetClaims(User user, IList<string> roles, string? orderId)
    {
        ...
        //{
        //    claims.Add(new Claim("orderId", orderId));
        //}
    
        if(!string.IsNullOrEmpty(orderId))
        {
            claims.Add(new Claim("orderId", orderId.ToString()));
        }
    ...
    public string CreateToken(User user, IList<string> roles,string? orderId)
    {
        var claims = GetClaims(user, roles, orderId);
